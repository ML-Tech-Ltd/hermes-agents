...
;; (loop repeat 10 do (gen-rules 1))

(defun get-tp-sl (rates &optional (max-size 10))
  (let* ((init-rate (first rates))
	 (max-pos 0)
	 (max-neg 0))
    (loop for rate in (subseq rates 0 max-size)
       do (let ((delta (- rate init-rate)))
	    (when (> delta max-pos)
	      (setf max-pos delta))
	    (when (< delta max-neg)
	      (setf max-neg delta))))
...
(defun drop-old-tests (amount unit timeframe)
  (with-postgres-connection (execute (delete-from :tests
				       (where (:and (:< :creation-time
  				       			(local-time:timestamp-to-unix
  				       			 (local-time:timestamp- (local-time:now)
  				       						amount unit)))
				       		    (:= :timeframe
  				       		    	(format nil "~a" timeframe))
				       		    ))
				       ))))
;; (drop-old-tests 9 :day :D)
;; (drop-old-tests 1 :minute :H1)
  ...
  (let* ((*rates* rates)
    ...
    `((:performance-metrics . ((:mae . ,mae)
      ...
      (:forecast (:delta . ,(last-elt sim))
		 (:entry-price . ,(if (> (last-elt sim) 0)
				      (access (alexandria:last-elt rates) :close-bid)
				      (access (alexandria:last-elt rates) :close-ask)))
